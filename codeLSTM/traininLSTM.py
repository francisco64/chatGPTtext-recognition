...
def classifier():
    ...
    prompt_features =Dropout(0.2)( LSTM(128)(prompt_features))
    
    
    txt_features =Dropout(0.2)( LSTM(128)(txt_features))
...
trainPrompt,traintxt,y_train=toArrays(train_d1+train_d2)

valPrompt,valtxt,y_val=toArrays(val_d1+val_d2)

model=classifier()
model.fit([trainPrompt,traintxt],y_train,epochs=100, batch_size=100,validation_data=([valPrompt,valtxt],y_val),callbacks=[tensorboard_callback,checkpoint])#,class_weight=class_weight)
